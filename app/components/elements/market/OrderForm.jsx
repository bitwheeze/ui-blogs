...
class OrderForm extends React.Component {
    ...
    updateTotal = (amount, price) => {
        ...
        total.amountFloat = totalFloat.toFixed(total.precision)
        return total
    }
    ...
    onTotalChange = e => {
        ...
        if (total.hasChange && total.asset.amount >= 0) {
            const { price } = this.state
            const amount = this.updateAmount(total.asset, price.float)
            this.setState({
                total,
    ...
    onBalanceClick = e => {
        ...
        if (isSell) {
            ...
            total = balance.clone()
            amount = this.updateAmount(total, this.state.price.float)
        }
        this.setState({
            amount: AssetEditor(amount),
            total: AssetEditor(total)
